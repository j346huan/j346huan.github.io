...
def _adjoin_variables_to_LPR(LPR, m, name='u'):
    r"""
    Adjoins new variables $u_1,u_2,...,u_{m}$ to the LaurentPolynomialRing `R` with
    ``name``. Return the ring and the new variables.
    """
    original_vars = list(LPR.variable_names())
    new_vars = [name+'%s' % (i+1) for i in range(m)]
    new_ring = LaurentPolynomialRing(LPR.base_ring(), new_vars + original_vars)
    return new_ring, new_ring.gens()[:m]

def taylor_reciprocal(f, prec, b):
    coeff = f.coefficients()
    if coeff[0][0]==0:
        print(coeff)
        raise NotImplementedError("%s" % R)
    leading_degree = coeff[0][1]
    leading_coeff = coeff[0][0]
    for i in range(len(coeff)):
        coeff[i][0]=pretty(coeff[i][0]/leading_coeff)
        coeff[i][1]=coeff[i][1]-leading_degree

    if prec>len(coeff)-1:
        coeff+=[[0,0] for i in range(prec-len(coeff)+1)]

    new_coeff = [[1,0]]
    for i in range(prec):
        new_coeff += [[-1*pretty(sum(new_coeff[j][0]*coeff[i+1-j][0] for j in range(i+1))),i+1]]

    for i in range(len(new_coeff)):
        new_coeff[i][0]=pretty(new_coeff[i][0]/leading_coeff)
        new_coeff[i][1]=new_coeff[i][1]-leading_degree

    new_f = sum(new_coeff[i][0]*(b^new_coeff[i][1])for i in range(len(new_coeff)))
    
    return new_f

def deg_at_most_n(f,n,b):
    L=f.coefficients()
    coeff =[(L[i] if L[i][1]<=n else [0,0])  for i in range(len(L))]
    return sum(coeff[i][0]*(b^coeff[i][1])for i in range(len(coeff)))
