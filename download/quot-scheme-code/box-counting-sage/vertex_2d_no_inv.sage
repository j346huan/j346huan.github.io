...
def tangent(p,x=x,y=y):
    ...
    for i in range(1,r+1):
        for j in range(1,r+1):
            for s in range((0 if j>=len(p.conjugate()) else p.conjugate()[j]),p.conjugate()[j-1]):
                total+=x^(i-j-1)*y^(p.conjugate()[i-1]-s-1)+x^(j-i)*y^(s-p.conjugate()[i-1])
...
def Verlinde_Num_custom(n):
    total=0
    for p in Partitions(n):
        add=measure_unsymmetrized(tangent(p))*\
            determinant_vb(p,n)*\
            determinant(canonical_bundle(p,x=x,y=y)-structure_sheaf(p,x,y))
        total+=add
    return total
